...
var rEvents;
var nEvents;
var nEvents_ar = new Array();
	
function fcontrolEvents(){
	nEvents_ar = [];
	if(openNav){
		$('#btSigOff').hide();
	} else {
		$('#btSigOff').show();
			if($('#elcuerpo').find('.boton_flotante').length>0){
				rEvents = true;
				if(pageEvents[npagina] == 1){
					$('#btSigOff').hide();
				} else {
					$('#elcuerpo').find('.boton_flotante').each(function(){
						nEvents_ar.push(0);
					});
					nEvents =  nEvents_ar.length;
				}
			} else if($('#elcuerpo').find('.acronym').length>0){
				rEvents = true;
				if(pageEvents[npagina] == 1){
					$('#btSigOff').hide();
				} else {
					$('#elcuerpo').find('.acronym').each(function(){
						nEvents_ar.push(0);
					});
					nEvents =  nEvents_ar.length;
				}
			} else if($('#elcuerpo').find('#autoevaluacion').length>0){
				rEvents = true;
				if(pageEvents[npagina] == 1){
					$('#btSigOff').hide();
				} else {
			...
			} else if($('#elcuerpo').find('#config_drag').length>0){
				rEvents = true;
				if(pageEvents[npagina] == 1){
					$('#btSigOff').hide();
				} else {
		...
		var id_enlace_ar = new Array();
		id_enlace_ar = [];
		$('#elcuerpo').each(function(){
			$(this).find('a').each(function(s){
				rEvents = true;
				if(pageEvents[npagina] == 1){
					$('#btSigOff').hide();
				} else {
					id_enlace_ar.push(nEvents_ar.length);
					nEvents_ar.push(0);
					nEvents = nEvents_ar.length;
					$(this).attr('id','link_'+s);
					$(this).bind('click',function(){
						nEvents_ar[id_enlace_ar[s]] = 1;
						activeEvents();
					})
				}
			});	
		...
		var id_pestanias_ar = new Array();
		id_pestanias_ar = [];
		$('#elcuerpo').find('#lista_pestanias').each(function(){
			$(this).find('li').each(function(nn){
				rEvents = true;
				if(pageEvents[npagina] == 1){
					$('#btSigOff').hide();
				} else {
					id_pestanias_ar.push(nEvents_ar.length);
					nEvents_ar.push(0);
					nEvents_ar[0] = 1;
					nEvents = nEvents_ar.length;
					$(this).bind('click',function(){
						nEvents_ar[id_pestanias_ar[nn]] = 1;
						//nEvents_ar[nn] = 1;
						activeEvents();
					})
				}
		...
		if(!rEvents){
			pageEvents[npagina] = 1;
			$('#btSigOff').hide();
		}
		
	}
	if((globalPagNext==undefined)||(globalPagNext==null)){
		$('#btSigOff').hide();
	}
}
...
function activeEvents(){
	var activSig = true;
	$.each(nEvents_ar,function(item,valor){
		if(valor == 0){
			activSig = false;
		}
	});
...
function fboton_flotante(){
	...
	$('#elcuerpo').find('.boton_flotante').each(function(n){
		...
		$(this).find('img').bind(clickEvent,function(){
			...
			} else if(tipo_ar[n]=="info"){
				// show info
				openShow_info(enlace_ar[n]);
...
function facronym(){
	...
	var tipoconte_ar = new Array();
	var ndatos = new Array();
	$('#elcuerpo').find('.acronym').each(function(s){
		datosBoton_ar.push($(this).attr('name'));
	});
	...
	$('#elcuerpo').find('.acronym').each(function(n){
		$(this).css({
				'left': posicionX_ar[n]+'px',
				'top': posicionY_ar[n]+'px'
		});
...
function fboton_extra(){
	$('#elcuerpo').find('.boton_extra').each(function(n){
		...
		$(this).find('img').bind(clickEvent,function(){
			...
			} else if(data_boton[0]=="info"){
				// show info
				openShow_info(data_boton[1]);
...
function creaShow_info(){
	$('#elcuerpo').find('.show_info').each(function(){
		data_info = [];
		if($(this).attr('name')){
			data_info = $(this).attr('name').split(',');
		}
		$(this).css({
			'left' : data_info[0]+'px',
			'top' : data_info[1]+'px',
			'width' : data_info[2]+'px'
		});
	});
}

function openShow_info(data){
	$('#'+data).show();
	$('#'+data).animate({
		'opacity' : '1.0'
	},500,function(){
	...
	$('#elcuerpo').find('.ventana_popup').each(function(n){
		...
		if(data_popup.length>0){
			...
			$(this).find('.contenido_popup').css({
				'width' : Math.round(Number(data_popup[2])-70)+'px'
			});
			...
			$(this).find('.boton_cerrar_popup').css({
				'left' : Math.round(Number(data_popup[2])-40)+'px'
			});
		}
		/*******************************************************************/
...
function creaVideo(){
	...
	if(user_navegador=="chrome"){
		...
		videojs("playerVideo"+rnd).ready(function() {
...
function creaGaleria(){
	...
	$('#elcuerpo').prepend('<div id="back_galeria"><img src="contenidos/img/back_galeria.png"/></div>');
	$('#back_galeria').css({
		'position' : 'absolute',
		'top' : '0px',
		'left' : '0px'
	...
	$('#galeria_fotos').parent().append('<div id="navegaFotos"><div id="flechaIzq"><img src="contenidos/img/fotos_fcIzq.gif"/></div><div id="flechaDch"><img src="contenidos/img/fotos_fcDch.gif"/></div></div>');
	$('#galeria_fotos').css({
			'opacity' : '0.3'
	});
...
function fdiv_flotante(){
	...
	$('#elcuerpo').find('.div_flotante').each(function(s){
		$(this).css({
			'left' : posX_ar[s]+'px',
			'top' : posY_ar[s]+'px',
			'width' : Math.round(936-posX_ar[s])+'px'
		});
...
function ffooter(){
	...
	$('#elcuerpo').find('.footer').each(function(){
		dataDiv_ar.push($(this).attr('name'));
	});
	...
	$('#elcuerpo').find('.footer').each(function(s){
		$(this).css({
			'left' : posX_ar[s]+'px',
			'top' : posY_ar[s]+'px'
		});
...
function fcloseTool(){
	$('#close_tool').bind(clickEvent, function(){
		$(this).unbind(clickEvent);
		callpage2(loadPage);
	});
}
