...
touchslider = {
	...
	//alert(nItem);
	//alert("pasa");
	if(touchslider.startLeft>touchslider.getLeft(elem)){
		if(nItem<(nFotos-1)){
			nItem++;
		}
		...
		touchslider.doSlide(elem, posx_fotosGlobal_ar[nItem], '0.5s');
	...
	stopSlide: function(elem){
		//alert("pasa"+elem);
		//alert(elem.css('left'));
		//alert($(gridid).attr('id'));	
		// DESACTIVAR TODAS LAS OPCIONES DE  makeTouchable
